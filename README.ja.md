...
[cc-sdd](https://github.com/gotalab/cc-sdd) のような優れたマルチAIツール対応の仕様駆動開発ツールがある中で、**claude-sdd** は異なる優先事項に焦点を当てています：

### シンプルさと移植性
- **NPM依存なし**: 単一の `curl` コマンドまたはローカルスクリプトでインストール
- **クロスプラットフォーム**: BashとPythonインストーラーがmacOS、Linux、Windows (WSL2) で動作
- **単一プロジェクト重視**: マルチツールワークフローではなく、個別のClaude Codeプロジェクトに最適化

### 柔軟性と設定
- **動的パス解決**: `spec-config.json` でカスタムディレクトリパスを設定
- **レガシーパスサポート**: `.kiro/` パスの自動検出と移行
- **JSONスキーマ検証**: 包括的なエラーメッセージを含む設定ファイル検証

### セキュリティと検証
- **組み込みパス検証**: ディレクトリトラバーサルと絶対パス脆弱性を防止
- **入力サニタイゼーション**: コマンドインジェクションを防ぐための機能名サニタイズ
- **セキュリティドキュメント**: `docs/security.md` での包括的なセキュリティガイドライン

### 強化された開発者体験
- **GitHub Issue統合**: `/spec:init-issue` でGitHub Issueから直接仕様を初期化
- **セッション状態管理**: resume機能付きの自動進捗追跡
- **トークン効率**: 最小限のコンテキストでクイックセッション復旧する `/spec:resume` コマンド
- **包括的テストガイド**: CI/CDテンプレート付き717行のテストドキュメント

### Claude Code ネイティブ
- **Claude Code最適化**: Claude Codeの独自機能をフル活用
- **スラッシュコマンド規則**: 一貫性のための `/spec:*` 名前空間使用
- **エラーハンドリング**: アクション可能な次のステップを含む包括的なエラーメッセージ

**cc-sdd を選ぶべき場合**: マルチAIツール/多言語チームワークフロー
**claude-sdd を選ぶべき場合**: Claude Code重視、移植性、セキュリティ、単一プロジェクトワークフロー

## 謝辞

このプロジェクトは、複数のAIコーディングアシスタント向けの優れた仕様駆動開発ツールである [cc-sdd](https://github.com/gotalab/cc-sdd) からインスピレーションを受けています。claude-sdd は仕様駆動開発手法において類似した概念的基盤を共有していますが、以下に焦点を当てて独立して実装されています：

- 単一プロジェクトの移植性とシンプルさ（NPM配布ではなくGitベースインストール）
- JSONスキーマ検証による動的パス設定
- Claude Code固有の最適化とネイティブ統合
- 強化されたセキュリティ機能（パス検証、入力サニタイゼーション）
- GitHub Issue統合とセッション状態管理

cc-sdd チームには、AIコーディングアシスタントエコシステムにおける仕様駆動開発アプローチの先駆けとして感謝いたします。両プロジェクトはMITライセンスを共有し、構造化された仕様駆動開発を開発者にとってアクセス可能にするという共通の目標を持っています。
