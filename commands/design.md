...
- **Context Loading**: Read `$SPECS_DIR$1/requirements.md`, core steering documents, and existing design.md (if merge mode)
...
- Verify alignment with core steering documents (`structure.md`, `tech.md`, `product.md`) and any custom steering files
  - **Core steering**: @$STEERING_DIR/structure.md, @$STEERING_DIR/tech.md, @$STEERING_DIR/product.md
...
- **Error Message**: "Requirements must be approved before generating design. Run `/spec:requirements $1` to review requirements, then run `/spec:design $1 -y` to proceed."
- **Alternative**: "Or run `/spec:design $1 -y` to auto-approve requirements and generate design."

### Conversation Guidance
After generation:
- Guide user to review design narrative and visualizations
- Suggest specific diagram additions if needed
- Direct to run `/spec:tasks $1 -y` when approved
